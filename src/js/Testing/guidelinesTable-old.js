...
class GuidelinesTable extends React.Component{
	...
	componentDidMount() {
  	...
  	selectGuidelinesByLetter = (letter)=>{
		...
		//Calls an asynchronous action generator getGuidelineData
  	...
  	selectGuidelinesByAuthor = (firstname, surname) =>{
	...
	render(){
		...
		const guideItemsAdmin = this.props.mappedGuidelines.map(g=>{
			...
			return(
				...
				<td style={{'padding-left':'5px'}}>{g.guideline_category}</td>
				<td><a href={g.file.url}>{g.guideline_title}</a></td>
				<td><a onClick={()=>console.log('item clicked')}>{g.guideline_description}</a></td>
				<td><a href={baseURLs.REACT_EDITOR_URL + '/' + g.id}>{g.status}</a></td>
				<td>End</td>
				</tr>)
